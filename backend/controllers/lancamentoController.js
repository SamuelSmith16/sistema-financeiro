...
exports.criarLancamento = async (req, res) => {
    try {
        ...
        if (!descricao || typeof descricao !== "string" || descricao.trim() === "") {
            return res.status(400).json({ error: "Descrição é obrigatória e não pode estar vazia" });
        }

        if (!data || !categoria || !tipo || valor === undefined) {
            return res.status(400).json({ error: "Campos obrigatórios: data, categoria, tipo, valor" });
        }
